...
export async function getStats(req, res) {
  try {
    ...
    const latestLogByMedicationDay = new Map();

    logs.forEach((log) => {
      latestLogByMedicationDay.set(`${log.date}:${log.medicationId}`, log);
    });

    const latestLogs = Array.from(latestLogByMedicationDay.values());
    ...
    const stats = Array.from({ length: 7 }, (_, index) => {
      ...
      const dayLogs = latestLogs.filter((log) => log.date === dateKey);
